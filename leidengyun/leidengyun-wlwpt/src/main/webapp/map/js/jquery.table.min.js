...
(function(sm, $) {
	sm.table = {
		...
		options  : {
			...
			columns : [],
			operate : [],
			pagination : {hide : false, pageSize : 100, pageSizes : [100, 1500, 3000, 4500]},
			after : function(){}
		},
		// 内部使用
		settings : {
		...
		createHeader :function () {
			...
			if(!sm.table.options.checkbox.hide){
				cell = $('<th/>').html('<label class="pos-rel"><input type="checkbox" class="ace" id="cc" /><span class="lbl"></span></label>').addClass('center');;
				cell.appendTo(row);
			}
		...
		bodyHandler: function(){
			...
			$('#cc').on('click', function(){
                ...
                var f =sel.checked;
                var food=document.getElementsByName('dd');
		...
		createBody :function () {
			...
			if(sm.table.settings.list.length > 0){
				for ( var i = 0, len = sm.table.settings.list.length; i < len; i++) {
					...
					// 添加checkbox
					if(!sm.table.options.checkbox.hide){
						cell = $('<td/>').html('<label class="pos-rel"><input type="checkbox" name="dd" class="ace" index="' + i + '"/><span class="lbl"></span></label>').addClass('center');
						cell.appendTo(row);
					}
					...
					for ( var j = 0, length = sm.table.options.columns.length; j < length; j++) {
						...
						if(column.replace){
							...
							if(column.field.indexOf(".")>-1){
								cell = $('<td/>').html((data[column.field.split(".")[0]][column.field.split(".")[1]] == null ? '' : data[column.field.split(".")[0]][column.field.split(".")[1]]) + '');
							}else{
