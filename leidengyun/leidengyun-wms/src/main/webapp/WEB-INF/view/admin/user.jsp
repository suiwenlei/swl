		...
		jQuery(function($) {
    		...
    		var $table = $("#_table").table({
				...
				tools : [
					...
					{text : '分配应用', clazz : 'btn-default', icon : 'fa fa-desktop grey', permission : '/admin/userApp/allocate', handler : function(){
						if(!$table.validateSelected(true)){
							return;						
						}
						$.aceRedirect("${_path}/admin/userApp/allocate?userId=" + $table.getSelectedItemKeys("id"));
					}},
					{text : '分配角色', clazz : 'btn-default', icon : 'fa fa-tachometer grey', permission : '/admin/userApp/allocate', handler : function(){
						if(!$table.validateSelected(true)){
							return;						
						}
						$.aceRedirect("${_path}/admin/userRole/allocate?userId=" + $table.getSelectedItemKeys("id"));
					}},
					{text : '分配设备', clazz : 'btn-default', icon : 'fa fa-laptop grey', permission : '/admin/userDev/allocate', handler : function(){
// 						if(!$table.validateSelected(true)){
// 							return;						
// 						}
				...
				operate : [
					...
					{text : '分配应用', clazz : 'grey', icon : 'fa fa-desktop', permission : '/admin/userApp/allocate', handler : function(d, i){
						$.aceRedirect("${_path}/admin/userApp/allocate?userId=" + d.id);
					}},
					{text : '分配角色', clazz : 'grey', icon : 'fa fa-tachometer', permission : '/admin/userRole/allocate', handler : function(d, i){
						$.aceRedirect("${_path}/admin/userRole/allocate?userId=" + d.id);
					}},
// 					{text : '分配设备', clazz : 'grey', icon : 'fa fa-laptop', permission : '/admin/userDev/allocate', handler : function(d, i){
// 						$.aceRedirect("${_path}/admin/userDev/allocate?userId=" + d.id);
// 					}},
				],
				after : function(){
					// 权限处理
